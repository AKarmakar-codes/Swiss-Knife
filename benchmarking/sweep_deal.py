"""
Hyperparameter Sweep Runner for DeAL
====================================

This script performs an end-to-end incremental sweep over the reward weight 
(`alpha`) for the DeAL baseline strategy (simplified with lookahead=0). It 
generates responses for 30 prompts across 6 different hyperparameter values 
to trace a Pareto frontier of Alignment vs. Quality.

How it works:
1. **Generation**: It iterates through the specified `alpha` values. For each value, 
   it initializes the `DeALGenerator` and generates responses for 30 prompts from 
   the HH-RLHF dataset.
2. **Storage**: The responses for each hyperparameter configuration are saved as 
   distinct `.jsonl` files (e.g., `deal_alpha_1.0.jsonl`) in the tribunal input folder.
...
Usage:
    python benchmarking/sweep_deal.py
"""
...
logger = logging.getLogger("sweep_deal")


def parse_args():
    p = argparse.ArgumentParser(description="Hyperparameter Sweep for DeAL")
...
def main():
    ...
    print("=" * 80)
    print("  DeAL Hyperparameter Sweep (Grid Search)")
    print("=" * 80)
    ...
    if not args.skip_generation:
        ...
        for alpha in alpha_values:
            ...
            out_file = os.path.join(output_dir, f"deal_alpha_{alpha}.jsonl")
