"""
Swiss-Knife — Harmlessness Benchmark on Anthropic HH-RLHF
===========================================================

Step-level decoding-time alignment benchmark focused on **harmlessness**.
Evaluates a set of alignment strategies on the Anthropic/hh-rlhf
harmless-base subset, using a DPO-trained Harmlessness Blade as the
reward signal during candidate selection.

What this script does:
    - Samples prompts from Anthropic/hh-rlhf's harmless-base test split.
      (https://huggingface.co/datasets/Anthropic/hh-rlhf/tree/main/harmless-base)
    - Generates responses under each strategy using a shared Drafter + Blade.
    - Does NOT score with an external reward model at inference time.
      Raw outputs are saved so a separate judge (Tribunal / GEval / AQI)
      can evaluate all strategies on identical generations.
    - Logs steering-verification diagnostics per response and per strategy:
      per-step blade reward stats, draft-vs-blade override rate, and a
      refusal-marker heuristic. See compute_steering_diagnostics() —
      these quantify whether the blade is steering generation independently
      of any downstream judge metric.

Strategies compared:
    1. baseline                 — Greedy decoding on base model (no alignment)
    2. baseline_adapter         — Greedy decoding with harmlessness LoRA adapter
    3. baseline_adapter_softmax — Softmax sampling with harmlessness LoRA adapter
    4. softmax_blade            — Softmax(β·r̃) step selection with blade reward
    5. pairwise_blade           — Bradley-Terry pairwise selection: P(A>B) = σ((r_A−r_B)/τ)
    6. swiss                    — Swiss-system points table → softmax champion
    7. elo_swiss                — Elo-rating tournament selection (Mode A, with acceptance gate)
    8. swiss_mode_b             — Swiss-system Mode B (unconditional acceptance, no fallback)
    9. elo_swiss_mode_b         — Elo-rating tournament Mode B (proposed strategy, unconditional)
    10. gumbel_topk             — Speculative Gumbel-Top-k sampling with blade fallback

Run:
    python evaluation/benchmark_HH_harmlessness.py \\
        --strategies baseline softmax_blade swiss elo_swiss swiss_mode_b elo_swiss_mode_b baseline_adapter_softmax \\
        --num-prompts 100 \\
        --n 8 \\
        --beta 0.1 \\
        --max-tokens 200 \\
